...
int visit_BlockNode(BlockNode_t *node, SYMT *symtable)
{
    if (node->declarations)
        visit(node->declarations, symtable);
...
void free_node(void *node)
{
    ...
    if (*((AST_NODE_TYPE *) node) == NODE_COMPOUND) {
        for (LIST_ENTRY *ptr = ((CompoundNode_t *) node)->statement_list; ptr;
             ptr = ptr->next) {
            free_node(ptr->data);
        }
    ...
    if (*((AST_NODE_TYPE *) node) == NODE_VAR_DECL) {
        for (LIST_ENTRY *outer = ((VarDeclNode_t *) node)->declaration_list;
             outer; outer = outer->next) {
            for (LIST_ENTRY *inner = outer->data; inner; inner = inner->next) {
